...
def main_pt():
    ...
    sparse_encoder = encoder.build_sparse_encoder(args.model, args.input_size, sbn=args.sbn)
    decoder = LightDecoder(up_sample_ratio=sparse_encoder.downsample_raito, sbn=args.sbn)

    model_without_ddp = PCFM(
        sparse_encoder=sparse_encoder, dense_decoder=decoder, mask_ratio=args.mask,
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            misc.save_checkpoint(f'{args.model}_withdecoder_1kpretrained_pcfm_style.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
