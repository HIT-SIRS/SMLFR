...
from low_freq import LowFreqTargetGenerator

class PCFM(nn.Module):
    def __init__(
            self, sparse_encoder, dense_decoder,
        ...
        for i in range(
            ...
            if i == 0 and e_width == d_width:
                ...
                print(f'[PCFM.__init__, densify {i + 1}/{self.hierarchy}]: use nn.Identity() as densify_proj')
            else:
                kernel_size = 1 if i <= 0 else 3
                densify_proj = nn.Conv2d(e_width, d_width, kernel_size=kernel_size, stride=1, padding=kernel_size // 2,
                                         bias=True)
                print(
                    f'[PCFM.__init__, densify {i + 1}/{self.hierarchy}]: densify_proj(ksz={kernel_size}, #para={sum(x.numel() for x in densify_proj.parameters()) / 1e6:.2f}M)')
        ...
        print(f'[PCFM.__init__] dims of mask_tokens={tuple(p.numel() for p in self.mask_tokens)}')
    ...
    def mask(self, B: int, device, generator=None):
        ...
        return torch.zeros(B, h * w, dtype=torch.bool, device=device).scatter_(dim=1, index=idx, value=True).view(B, 1,
                                                                                                                  h, w)

    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False):
        # step1. Generate: get low freq target
        low_freq = LowFreqTargetGenerator(radius=40, img_size=224)
        target_bchw = low_freq(inp_bchw)
        ...
        active_b1hw = active_b1ff.repeat_interleave(self.downsample_raito, 2).repeat_interleave(self.downsample_raito,
                                                                                                3)  # (B, 1, H, W)
        masked_bchw = inp_bchw * active_b1hw

        # step3. Encode: get hierarchical encoded sparse features (a list containing 4 feature maps at 4 scales)
        ...
        # step4. Densify: get hierarchical dense features for decoding
        ...
        # step5. Decode and reconstruct
        rec_bchw = self.dense_decoder(to_dec)
        target, rec = self.patchify(target_bchw), self.patchify(
            rec_bchw)  # inp and rec: (B, L = f*f, N = C*downsample_raito**2)
        mean = target.mean(dim=-1, keepdim=True)
        var = (target.var(dim=-1, keepdim=True) + 1e-6) ** .5
        target = (target - mean) / var
        l2_loss = ((rec - target) ** 2).mean(dim=2, keepdim=False)  # (B, L, C) ==mean==> (B, L)
    ...
    def __repr__(self):
        return (
            f'\n'
            f'[PCFM.config]: {pformat(self.get_config(), indent=2, width=250)}\n'
            f'[PCFM.structure]: {super(PCFM, self).__repr__().replace(PCFM.__name__, "")}'
        )
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False, with_config=False):
        state = super(PCFM, self).state_dict(destination=destination, prefix=prefix, keep_vars=keep_vars)
        if with_config:
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        incompatible_keys = super(PCFM, self).load_state_dict(state_dict, strict=strict)
...
if __name__ == '__main__':
    ...
    sparse_encoder = encoder.build_sparse_encoder('convnext_base', input_size=224, sbn=False).cuda()
    decoder = LightDecoder(up_sample_ratio=32, width=768, sbn=False).cuda()
    model = PCFM(sparse_encoder=sparse_encoder, dense_decoder=decoder).cuda()

    recon_loss = model(img)
    ...
    # print(encoder._cur_active.shape)
