...
class RotatedVariedSizeWindowAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        qkv = F.linear(input=shortcut.permute(0, 2, 3, 1).reshape(b, -1, self.dim), weight=self.qkv.weight,
                       bias=qkv_bias)

        qkv = qkv.permute(0, 2, 1).reshape(b, -1, h, w).reshape(b, 3, self.num_heads, self.out_dim // self.num_heads, h,
                                                                w).transpose(1, 0).reshape(3 * b * self.num_heads,
...
class NormalCell(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 tokens_type='transformer', kernel=3, mlp_hidden_dim=None, window_size=None, attn_head_dim=None,
                 window=False):
        super().__init__()
    ...
    def forward(self, x, H, W):
        ...
        if self.class_token:
            ...
            convX = self.drop_path(
                self.PCM(x[:, 1:, :].view(b, wh, wh, c).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3,
                                                                                                  1).contiguous().view(
                    b, n, c))
